/* ===========================================================================
   DAILY AGENT: high-income / high-cibil customers MISSED by ultra_power_waterfall_scrupg
   ===========================================================================

   WHAT THIS FILE IS
     This is Scirevawf_21_26August.sql (the Sicreva repeat-offer waterfall
     pipeline, Ring platform) with two changes:
       1. start_date/end_date/fat_start_date now resolve to YESTERDAY relative
          to whenever this runs, instead of a hardcoded historical range.
       2. STEP 6 is appended at the very end. It takes the same
          ...
          Results are appended to a permanent log table
          (kissht_reports.temp_tables.daily_hv_ultra_power_miss_log) keyed by
          run_date, so day-over-day trend/drift is queryable, and a
          Slack-ready summary is produced as the final result set.
     Everything above STEP 6 (Steps 1-5) is UNCHANGED pipeline logic — see
     the original comment block below for how it works and its gotchas.
     Run this once per day (e.g. via a scheduled agent) after yesterday's
     applications have settled.
   ===========================================================================
   ...
   =========================================================================== */
/* Daily agent: always resolve to YESTERDAY relative to run time. */
SET start_date     = (SELECT TO_VARCHAR(DATEADD(day, -1, CURRENT_DATE())));   /* yesterday, inclusive */
SET end_date       = (SELECT TO_VARCHAR(DATEADD(day, -1, CURRENT_DATE())));   /* yesterday, inclusive */
SET fat_start_date = (SELECT TO_VARCHAR(DATEADD(day, -3, CURRENT_DATE())));   /* ~2 days before start_date */
...
/* ═══════════════════════════════════════════════════════════════
   ...
   WHY : these are customers who look clearly credit-worthy on income/bureau
         ...
         policy is behaving as intended or whether an upstream feed/join is
         broken. This is exactly the ultra_power_waterfall_scrupg cascade
         from Step 5, just narrowed to this population and logged daily so it
         can be trended.
   NOTE: kept as one wide detail table (one row per missed application) plus
         a compact summary underneath. The detail table is append-only by
         run_date so history accumulates; re-running the same run_date
         replaces that day's rows only (safe to re-run).
   ═══════════════════════════════════════════════════════════════ */
CREATE TABLE IF NOT EXISTS kissht_reports.temp_tables.daily_hv_ultra_power_miss_log (
    run_date                    DATE,
    fb_transaction_id           VARCHAR,
    user_reference_number       VARCHAR,
    application_date            DATE,
    module                      VARCHAR,
    status                      VARCHAR,
    output_aa_income            FLOAT,
    bureau_cibil_3_score        FLOAT,
...
SELECT
    ...
    w.fb_transaction_id,
    w.user_reference_number,
    w.date                                      AS application_date,
    w.module,
    w.status,
    w.output_aa_income,
    w.rv_bureau_cibil_3_score                   AS bureau_cibil_3_score,
    w.user_gross_monthly_salary,
    ARRAY_TO_STRING(ARRAY_CONSTRUCT_COMPACT(
        CASE WHEN w.output_aa_income > 40000                THEN 'aa_income>40k'   END,
        CASE WHEN w.rv_bureau_cibil_3_score > 760            THEN 'cibil>760'       END,
...
ORDER BY customers_missed DESC;
